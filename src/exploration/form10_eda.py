...
def read_form(path):
    ...
    """
    form_df = pd.read_csv(path).set_index("WorkerID")
    form_df = form_df.rename(
        columns={col_name: col_name.strip() for col_name in form_df.columns}
    )
...
def user_representation(formdf, w2v):
    ...
    em_reps = pd.DataFrame(em_reps, columns=["em", "reps"]).set_index("em")
    return em_reps
...
def unif_representation(form1_df, w2v):
    ...
    em_reps_df = pd.DataFrame(em_reps, columns=["em", "reps"]).set_index("em")
    return em_reps_df
...
def compute_em2v_vecs(df, e2v):
    ...
    df["corr_mean"] = df[["mean_rep", "em2vec_rep"]].apply(
    ...
    df["corr_median"] = df[["median_rep", "em2vec_rep"]].apply(
        lambda x: np.corrcoef(x[0], x[1])[0, 1], axis=1
    )
...
def plot_corr(df, agg_type="mean", ax=None):
    ...
    ax.set_title(
        f"Correlations between {agg_type} of crowdsourced rep and emojivec rep"
    )
...
def get_words_scores(form1_df, user_df, w2v, col="mean_rep"):
    ...
    words_scores_df = (
        words_scores_df.apply(lambda x: list(set(x.split(","))))
        .reset_index()
    ...
    words_scores_df["wordvec"] = words_scores_df["word"].apply(
    ...
    words_scores_df["word_cossim"] = words_scores_df[["wordvec", col]].apply(
        lambda x: (1 - spatial.distance.cosine(x[0], x[1])), axis=1
    )
...
def get_words_counts(form1_df, em):
    ...
    word_counts = pd.Series(
        form1_df.transpose().agg(",".join, axis=1).loc[em].split(",")
    ).value_counts()
    print(f"Emoji:{em}:")
