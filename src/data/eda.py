...
def get_emoji_df(e2v):
    """ create the emoji-vec dataframe"""
    em_df = [{"index":i,"em":em,"vec":e2v.get_vector(em)} for i,em in mapping.items()]
    em_df = pd.DataFrame(em_df).set_index("index")
    return em_df

def get_10_faces(em,num_faces=5):
    """returns the top 10 most similar faces"""
    topn = num_faces
    faces = [i[0] for i in e2v.similar_by_word(em,topn=topn)]
    while not (all([face in emotions_faces for face in faces]) and len(faces) == num_faces):
        topn += 1
        faces = [i[0] for i in e2v.similar_by_word(em,topn=topn)]
