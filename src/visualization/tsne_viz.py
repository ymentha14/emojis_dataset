...
def tsne_plot(
    ...
    if os.environ.get("DEBUG") is not None:
        n_iter = 500
    tsne = man.TSNE(perplexity=30, n_components=2,
                    init="pca", n_iter=n_iter, verbose=1)
...
def main(model_type, embedding_type, use_faces=False, use_words=False):
    ...
    if embedding_type == "bert":
        if model_type == "em_dataset":
            input_path = EXPORT_DIR.joinpath(
                "data/embeddings/bert/em_dataset.pk")
            ...
            mapping_path = EXPORT_DIR.joinpath(
                "data/embeddings/word2vec/e2v/mapping.pk"
            )
        ...
        if model_type == "e2v":
            ...
            input_dir = EXPORT_DIR.joinpath(
                "data/embeddings/word2vec/em_dataset/")
